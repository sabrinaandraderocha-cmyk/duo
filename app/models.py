...
from sqlalchemy import (
    Column, Integer, String, ForeignKey, Text, Index
)
...
class User(Base):
    ...
    id = Column(Integer, primary_key=True)
    couple_id = Column(Integer, ForeignKey(fk("couples"), ondelete="SET NULL"), nullable=True)

    name = Column(String(80), nullable=False)
    email = Column(String(160), nullable=False)
    password_hash = Column(String(255), nullable=False)
    
    # Campo novo (vamos adicionar via SQL depois)
    recovery_key = Column(String(100), nullable=False, default="") 

    couple = relationship("Couple", back_populates="users")

# =========================
# DIÁRIO (Nome original: entries)
# =========================
class Entry(Base):
    __tablename__ = "entries"
    
    # IMPORTANTE: Sem a UniqueConstraint antiga
    __table_args__ = table_args(
